...
class AlbertAttention(BertSelfAttention):
    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None):
        mixed_query_layer = self.query(input_ids)## 生成query [16,32,768],16是batch_size,32是这个batch中每个句子的长度，768是维度
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    Return:
        ...
        last_hidden_state (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_lengtgh, hidden_size)`):
...
class AlbertForSequenceClassification_dpcnn(AlbertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config.output_hidden_states = True
        # add cnn params
        self.filter_sizes_ = (3, 4, 5)  # 卷积核尺寸
        config.num_filters = 250  # 卷积核数量(channels数)
        ...
        self.relu = nn.ReLU()
        # self.classifier = nn.Linear(config.num_filters*1 + config.hidden_size, config.num_labels)
        self.classifier = nn.Linear(config.num_filters*4, config.num_labels)
    ...
    def forward(
        ...
        x1 = outputs[2][-1].unsqueeze(1)  # [batch_size, 1, seq_len, embed]
        ...
        while x1.size()[2] >= 2:
            x1 = self._block(x1)
        x1 = x1.squeeze(2)  # [batch_size, num_filters(250)]
        x1 = x1.squeeze(2)  # [batch_size, num_filters(250)]



        x2 = outputs[2][-2].unsqueeze(1)  # [batch_size, 1, seq_len, embed]
        ...
        while x2.size()[2] > 2:
            x2 = self._block(x2)
        x2 = x2.squeeze(2)  # [batch_size, num_filters(250)]
        x2 = x2.squeeze(2)  # [batch_size, num_filters(250)]

        x3 = outputs[0].unsqueeze(1)  # [batch_size, 1, seq_len, embed]
        ...
        while x3.size()[2] > 2:
            x3 = self._block(x3)
        x3 = x3.squeeze(2)  # [batch_size, num_filters(250)]
        x3 = x3.squeeze(2)  # [batch_size, num_filters(250)]

        x4 = outputs[0].unsqueeze(1)  # [batch_size, 1, seq_len, embed]
        x4 = self.conv_region(x4)  # [batch_size, 250, seq_len-3+1, 1]

        x4 = self.padding1(x4)  # [batch_size, 250, seq_len, 1]
        x4 = self.relu(x4)
        x4 = self.conv(x4)  # [batch_size, 250, seq_len-3+1, 1]
        x4 = self.padding1(x4)  # [batch_size, 250, seq_len, 1]
        x4 = self.relu(x4)
        x4 = self.conv(x4)  # [batch_size, 250, seq_len-3+1, 1]
        while x4.size()[2] > 2:
            x4 = self._block(x4)
        x4 = x4.squeeze(2)  # [batch_size, num_filters(250)]
        x4 = x4.squeeze(2)  # [batch_size, num_filters(250)]
