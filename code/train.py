...
from transformers import (AlbertTokenizer, AlbertConfig,
                          AlbertForSequenceClassification,AlbertForSequenceClassification_dpcnn,
...
MODEL_CLASSES = {
    ...
    'albert':(AlbertConfig, AlbertForSequenceClassification, AlbertTokenizer),#0.71(batchsize=32,kfold=10);0.69(batchsize=32,kfold=15) 0.76(batchsize=16,kfold=10) 0.81(batchsize=8,kfold=10，a=0.00001)
    'albert-v2':(AlbertConfig, AlbertForSequenceClassification_dpcnn, AlbertTokenizer),#0.87(batchsize=16,kfold=10 学习率2e-5 0.86) #0.86(batchsize=8,kfold=10学习率调到2e-5变0.87) batchsize32爆炸 0.88(batchsize16 梯度累加4 学习率2e-5 15折) 0.89(batchsize16 学习率2e-5 梯度累加2 15折)
    'albert-v2':(AlbertConfig, AlbertForSequenceClassification_dpcnn, AlbertTokenizer),#0.87(batchsize=16,kfold=10 学习率2e-5 0.86) #0.86(batchsize=8,kfold=10学习率调到2e-5变0.87) batchsize32爆炸 0.88(batchsize16 梯度累加4 学习率2e-5 15折) 0.89(batchsize16 学习率2e-5 梯度累加2 15折)
    'albert-large':(AlbertConfig, AlbertForSequenceClassification, AlbertTokenizer)#batchsize16爆炸
...
def load_and_cache_examples(args, tokenizer, set_type='train'):
    ...
    #第二个核心操作是convert_examples_to_features讲数据进行转化
...
#6.训练模型，一个是加载需要的数据集，一个是进行训练
def train(args, tokenizer, model):
    """ Train the model """
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    train_dataset = load_and_cache_examples(args, tokenizer, set_type='train')#load_and_cache_examples 就是加载训练数据集
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)#对数据随机采样
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)#DataLoader读取数据，计算总共训练步数（梯度累计），warm_up 参数设定，优化器，是否fp16等等，然后一个batch一个batch进行训练
...
def main():
    ...
    #2.解析命令行参数
    parser.add_argument('--adv_type', default='None', type=str, choices=['fgm', None])
    ...
    #3.判断一些情况，判断单机cpu训练还是单机多gpu训练，还是多机分布式gpu训练
    ...
    #4.获取任务对应的相应processor，这个对应的函数就是需要我们自己去定义的处理我们自己输入文件的函数
    processor = PROCESSORS[args.task_name]()
    label_list = processor.get_labels()
